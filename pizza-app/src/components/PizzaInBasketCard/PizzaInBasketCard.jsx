...
const PizzaInBasketCard = ({dataPizzaItem, basketData, setBasketData, updateBasket, id}) => {
  ...
  const getAllToppingsString = (top) => {
    return top.reduce(function(acc, allToppings){
      return acc + allToppings.name + ", "
    },"+ ").replace(/,\s*$/, '');
  };
  ...
  return(
    <li className="pizza_in_basket_item">        
      ...
      <p className="pizza_in_basket_desc">{sizeWord} {size} см, традиционное тесто {!(allToppings.length === 0) ? getAllToppingsString(allToppings) : ""}</p>
